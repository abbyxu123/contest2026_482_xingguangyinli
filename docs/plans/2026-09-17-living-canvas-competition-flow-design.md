...
- SID is a backend dependency only. It supplies recommendation orchestration,
  model/rules fallback, confirmation state, and the phone handoff URL.
- Gemini S1 owns the complete visual language. No SID cat artwork, SID screen,
...
7. Show a compact QR code for the SID phone console.
...
| --- | --- | --- |
| Beagle | coordinate and execute the meal task | SID orchestration and explicit confirmation |
...
It turns user events and bounded SID results into display states. The existing
`lc_sid_contract` owns request/confirmation payloads. A later hardware adapter
...
SID remains the source of truth for recommendation and handoff. MiMo is the
primary model through the generic OpenAI-compatible `MODEL_*` configuration.
If the model is unavailable, SID uses deterministic rules and still completes
the flow.
...
- A phone scans the displayed QR and reaches the SID confirmation page.
