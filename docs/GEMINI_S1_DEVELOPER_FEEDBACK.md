...
5. Gemini-S1 板级 `POSTBUILD` 原先直接引用
   `prebuilts/gcc/linux-x86_64/.../arm-none-eabi-strip` 与 `objcopy`。在 ARM64
   构建主机上，目标源码已编译并链接成功，但最终固件转换会因宿主架构不匹配而
   失败。改用构建系统提供的 `$(CROSSDEV)strip` 与 `$(OBJCOPY)` 后，同一配置
   完成全量构建和 `vela.bin` 生成。
...
- 板级 `Make.defs` 的固件转换步骤使用 `$(CROSSDEV)strip`、`$(OBJCOPY)` 等
  已解析变量，不再拼接固定的 `linux-x86_64` 目录；
- 在 CI 中同时覆盖 x86_64 与 ARM64 Linux 主机，至少验证编译、链接、strip、
  objcopy 和目标整包生成；
- 构建失败时分别报告“目标代码失败”和“宿主后处理工具不可执行”，避免把已完成
  的目标链接误判为源码编译失败。
